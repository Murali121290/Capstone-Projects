...
variable "key_name" {
  ...
  type        = string
  default     = "murali26jul2025"   # <-- replace if needed
}

variable "private_key_path" {
  description = "Local path to the private key (.pem) used for SSH."
  type        = string
  default     = "/home/ubuntu/.ssh/murali26jul2025.pem"  # update for your system
}
...
resource "aws_instance" "dev_server" {
  ...
  user_data = <<-EOF
    ...
    # NOTE: Minikube requires manual start after SSH login:
